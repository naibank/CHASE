...
for (x in 1:nrow(gene_table)){
  ...
  gene_table[x,"proband_nonsyn"] <- nrow(SNVs_for_gene[which(SNVs_for_gene$Relation == "proband or affected sibling" &
                                                           SNVs_for_gene$effect_priority == "nonsynonymous SNV"),])
  gene_table[x,"proband_syn"] <- nrow(SNVs_for_gene[which(SNVs_for_gene$Relation == "proband or affected sibling" &
                                                         SNVs_for_gene$effect_priority == "synonymous SNV"),])
}

gene_table <- setorder(gene_table, -"proband_nonsyn")

write.table(gene_table, "SNV_count_per_gene.tsv", sep="\t", row.names=F, quote=F, col.names=T)
