...
write.table(CNV_SNV_table, "SSC_proband_CNV_SNV_table_eventfreq.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
for(freq in c(0.01, 0.005, 0.001, 0.0005, 0.0001)){ # event freq cut-offs
  ...
  for(pRecx in c(0, 0.5, 0.9)){ # pRec cut-offs
    ...
    for (row in 1:nrow(count_table)){
      ...
      if (count_table[row,"variant"] == "All"){
        if (count_table[row, "relation"] == "Probands"){
          count_table[row, "count"] <- nrow(unique(table[which(table$Relation == "proband"), c("#Sample", "#id")]))
        }
      ...
      if (count_table[row,"variant"] == "LoF"){
        if (count_table[row, "relation"] == "Probands"){
          count_table[row, "count"] <- nrow(unique(table[which(table$Relation == "proband"&
    ...
    plot_name =  paste("SSC_proband_eventfreq", freq, "_pRec", pRecx, "_trate", sep="")###Bank
    ...
    plot <- ggplot(trate_table, aes(x = variant_type, y = trate)) + 
      ...
      labs(title = plot_name, x = "Variant Type", y = "Transmisstion Rate") +
...
write.table(CNV_SNV_table, "SSC_unaffected_sibling_CNV_SNV_table_eventfreq.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
for(freq in c(0.01, 0.005, 0.001, 0.0005, 0.0001)){ # event freq cut-offs
  ...
  for(pRecx in c(0, 0.5, 0.9)){ # pRec cut-offs
    ...
    plot_name =  paste("SSC_unaffectedsibling_eventfreq", freq, "_pRec", pRecx, "_trate", sep="")
