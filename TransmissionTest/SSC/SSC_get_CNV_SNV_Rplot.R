...
metadata <- fread("SSC_metadata.tsv", data.table = F)
...
meta_parentsID <- metadata$`Sample ID`[which(metadata$Relation %in% c("father","mother"))]
...
for(freq in c(0.1, 0.05, 0.01, 0.001)){
  table <- fread("SSC_proband_CNV_SNV_table_noCRV.tsv", data.table = F)
  ...
  for(pRecx in c(0, 0.7, 0.9)){
    ...
    ggsave(sprintf("../TT/%s.png", plot_name), width = 10)}}


## plot unaffected sibling table
for(freq in c(0.1, 0.05, 0.01, 0.001)){
  table <- fread("SSC_unaffected_sibling_CNV_SNV_table_noCRV.tsv", data.table = F)
  ## add a column to indicate the relation
  table$Relation <- NA
  table$Relation[which(table$`#Sample` %in% meta_parentsID)] <- "parent"
