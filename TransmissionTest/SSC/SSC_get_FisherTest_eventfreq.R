...
for (relation in c("proband", "unaffected_sibling")){
  if (relation == "proband"){
    CNV_SNV_table <- fread("SSC_proband_CNV_SNV_table_eventfreq.tsv", data.table = F)

    child = "proband or affected sibling" 
  }
  if (relation == "unaffected_sibling"){
    CNV_SNV_table <- fread("SSC_unaffected_sibling_CNV_SNV_table_eventfreq.tsv", data.table = F)
  ...
  table_name <- paste("SSC_", relation, "_FisherTest_eventfreq_snvfreq0.01.tsv", sep = "")
  write.table(test.out, paste('../TT/Combined Data/SSC/SNV0.01/',table_name), sep="\t", row.names=F, quote=F, col.names=T)
}
