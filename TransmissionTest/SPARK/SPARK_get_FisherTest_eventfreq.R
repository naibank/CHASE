...
meta <- rbind(read.delim("./SPARK/SPARK_WGS_1_metadata_relfixed.tsv", stringsAsFactors = F),
              read.delim("./SPARK/SPARK_WGS_2_metadata_relfixed.tsv", stringsAsFactors = F),
              read.delim("./SPARK/SPARK_WGS_3_metadata_relfixed.tsv", stringsAsFactors = F))

for (relation in c("proband", "unaffected_sibling")){
  if (relation == "proband"){
    CNV_SNV_table <- fread("./SPARK/SPARK_proband_CNV_SNV_table_eventfreq.tsv", data.table = F)
    
    child = "proband or affected sibling" 
  }
  if (relation == "unaffected_sibling"){
    CNV_SNV_table <- fread("./SPARK/SPARK_unaffected_sibling_CNV_SNV_table_eventfreq.tsv", data.table = F)
  ...
  for(freq in c(0.01, 0.001, 0.005, 0.0005, 0.0001)){ # event freq cut-offs
    for(pRecx in c(0, 0.5, 0.9)){ # pRec cut-offs
      for(variant_type in c("damaging_missense", "LoF", "nonsynonymous","synonymous")){
        ...
        target.child <- nrow(unique(CNV_SNV_table[intersect(target.ix, which(CNV_SNV_table$Relation == child)), c("#Sample", "#id")]))
  ...
  table_name <- paste("SPARK_", relation, "_FisherTest_eventfreq_snvfreq0.01.tsv", sep = "")
  write.table(test.out, paste('../TT/Combined Data/SPARK/SNV0.01/',table_name), sep="\t", row.names=F, quote=F, col.names=T)
}
