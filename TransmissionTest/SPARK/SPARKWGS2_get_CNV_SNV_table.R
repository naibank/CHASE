...
path <- "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/TT/SNV/SPARKWGS2"
...
write.table(snvs,"SPARKWGS2_SNVs.tsv", sep="\t", row.names=F, quote=F, col.names=T)
snvs <- fread("SPARKWGS2_SNVs.tsv", data.table=F)
message('Done loading snvs')

## Get filtered metadata file:
metadata <- Filter_Metadata("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/Data/SPARK_WGS_2_metadata_relfixed.tsv", unique(snvs$`#Sample`))

cnv <- fread("SPARKWGS2.CNVs.freq.1perc.tsv", data.table = F)
...
write.table(proband_table, "SPARKWGS2_proband_CNV_SNV_table.tsv",  sep="\t", row.names=F, quote=F, col.names=T)
write.table(unaffectedsib_table, "SPARKWGS2_unaffected_sibling_CNV_SNV_table.tsv",  sep="\t", row.names=F, quote=F, col.names=T)
