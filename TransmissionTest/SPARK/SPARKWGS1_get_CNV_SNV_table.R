...
Get_Failed_QC_Samples <- function() {
  #MSSNG + SSC combined
  Subjects_to_exclude <- fread('/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/Data/excluded.qcfailed.samples.txt', header=F, col.names='Subjects_to_exclude')
...
Get_cr_CNV_Samples <- function() {
  #MSSNG + SSC combined
  Subjects_to_exclude <- fread('/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/Data/samples.with.crCNV.txt', header=F, col.names='Subjects_to_exclude')
...
path <- "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/TT/SNV/SPARKWGS1"
...
write.table(snvs,"SPARKWGS1_SNVs.tsv", sep="\t", row.names=F, quote=F, col.names=T)
snvs <- fread("SPARKWGS1_SNVs.tsv", data.table=F)
message('Done loading snvs')

## Get filtered metadata file:
metadata <- Filter_Metadata("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/Data/SPARK_WGS_1_metadata_relfixed.tsv", unique(snvs$`#Sample`))

cnv <- fread("SPARKWGS1.CNVs.freq.1perc.tsv", data.table = F)
...
tables <- Get_CNV_SNV_Tables(list(cnv), snvs, metadata)
...
unaffectedsib_table <- unaffectedsib_table[which(!unaffectedsib_table$Relation == "NA"),]

write.table(proband_table, "SPARKWGS1_proband_CNV_SNV_table.tsv",  sep="\t", row.names=F, quote=F, col.names=T)
write.table(unaffectedsib_table, "SPARKWGS1_unaffected_sibling_CNV_SNV_table.tsv",  sep="\t", row.names=F, quote=F, col.names=T)
