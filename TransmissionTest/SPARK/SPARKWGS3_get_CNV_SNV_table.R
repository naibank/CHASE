...
path <- "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/TT/SNV/SPARKWGS3"
...
write.table(snvs,"SPARKWGS3_SNVs.tsv", sep="\t", row.names=F, quote=F, col.names=T)
snvs <- fread("SPARKWGS3_SNVs.tsv", data.table=F)
message('Done loading snvs')

## Get filtered metadata file:
metadata <- Filter_Metadata("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARKAnalysis/Data/SPARK_WGS_3_metadata_relfixed.tsv", unique(snvs$`#Sample`))

cnv <- fread("SPARKWGS3.CNVs.freq.1perc.tsv", data.table = F)
...
write.table(proband_table, "SPARKWGS3_proband_CNV_SNV_table.tsv",  sep="\t", row.names=F, quote=F, col.names=T)
write.table(unaffectedsib_table, "SPARKWGS3_unaffected_sibling_CNV_SNV_table.tsv",  sep="\t", row.names=F, quote=F, col.names=T)
