...
path <- "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/exonic+splicing"
...
for(i in 1:total){
  ...
  if(nrow(snvs) > 10000000){
    write.table(snvs, sprintf("./SPARKWGS1.SNVs.freq._%s.tsv", i), sep="\t", row.names=F, quote=F, col.names=T)
    snvs <- data.frame()
  }
}
write.table(snvs, sprintf("./SPARKWGS1.SNVs.freq._%s.tsv", total), sep="\t", row.names=F, quote=F, col.names=T)
