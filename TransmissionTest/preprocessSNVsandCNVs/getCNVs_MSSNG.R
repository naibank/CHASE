...
tmp <- fread("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/MSSNG/CNVs.ILMN/CNVs.MSSNG_ILMN.freq_10percent.HQR.IntFreq.tsv", data.table = F)
tmp <- tmp[which(tmp$SVTYPE == "DEL" & tmp$Sample_QC == "ok" & tmp$Inheritance %in% c("Paternal", "Maternal") &
                 tmp$FILTER == "PASS" & tmp$freq_max/100 <= 0.1), 
           c("sample", "CHROM", "START", "END", "SVTYPE", "gene_symbol", "gene_egID", "exon_symbol", "exon_egID",
             ...
             "sscErdsPercFreq_50percRecipOverlap", "sscCnvnPercFreq_50percRecipOverlap", "Inheritance", "freq_max")]

write.table(tmp, "./MSSNG.CNVs.freq.1perc.tsv", sep="\t", row.names=F, quote=F, col.names=T)
