################################################################################################################################################################################
# 
# plot_CNV_SV_rate_comparison_by_individual_sizebins_SW.R
# purpose: - outputs MSSNG+SSC sliding window cut-offs (median, 80, 90 perc.) spline tables
#          - plots MSSNG+SSC sliding window cut-offs (median, 80, 90 perc.) spline
#          - plots histogram & violin plots for no. CH hits vs. no. individuals
# input: MSSNG & SSC CH_hits tables
#        /hpf/largeprojects/tcagstor/tcagstor_tmp/shania.wu/CH_count_del_size/data/CH_hits/
# output: MSSNG.SSC.hist.violin.*size range*.png, MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df,
#           all.cutoff_2.5kb window.n=7.pdf
#         /hpf/largeprojects/tcagstor/tcagstor_tmp/shania.wu/CH_count_del_size/figures/, ./histograms/
# 
# notes:
#
##############################################################################################################################################################################
...
## MSSNG+SSC bin size 500 bp cutoffs table
MSSNG.SSC.window.cutoffs.df <- Get_Sliding_CutOffs_Table(MSSNG.SSC.CH.hits, bin.end=30000, bin.size = 500)

## MSSNG+SSC bin size 2.5 kb cutoffs table
MSSNG.SSC.2.5kb.n7.window.cutoffs.df <- Get_Sliding_CutOffs_Table(MSSNG.SSC.CH.hits, 
                                                               bin.end=30000, bin.size = 2500,
                                                               window.bin.n = 7)
MSSNG.SSC.2.5kb.n5.window.cutoffs.df <- Get_Sliding_CutOffs_Table(MSSNG.SSC.CH.hits, 
                                                                  bin.end=30000, bin.size = 2500,
                                                                  window.bin.n = 5)

MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df <- Get_Sliding_CutOffs_Table(MSSNG.SSC.CH.hits.pRec, 
                                                                  bin.end=30000, bin.size = 2500,
                                                                  window.bin.n = 7)

# write.table(MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df, "./data/MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df",
...
Get_Sliding_Window_Cutoff_Plots <- function(window.cutoffs.df, CH.hits.df, file.name){
  ...
  pdf(file=sprintf("./figures/all.cutoff_%s.pdf", file.name), width=9, height=7)
  
  plot(x=sliding.window.df$size.bin.start, y = sliding.window.df$percentile_0.9, col = 1,
       main = sprintf("MSSNG+SSC Sliding Window Cut-offs (bin size = %s)",file.name), 
...
Get_Sliding_Window_Cutoff_Plots(MSSNG.SSC.2.5kb.n7.window.cutoffs.df, MSSNG.SSC.CH.hits, "2.5kb window.n=7")
Get_Sliding_Window_Cutoff_Plots(MSSNG.SSC.2.5kb.n5.window.cutoffs.df, MSSNG.SSC.CH.hits, "2.5kb window.n=5")


# ### Combine median, 80th, 90th percentile for sliding window approach ####
# MSSNG.SSC.median.mean.80.90 <- comb.median[,c("size.bin.start", "size.bin.end", "cohort")]
# MSSNG.SSC.median.mean.80.90$"median" <- comb.median$cut.off
# MSSNG.SSC.median.mean.80.90$"mean" <- comb.mean$cut.off
# MSSNG.SSC.median.mean.80.90$"80th.percentile" <- comb.0.8$cut.off
# MSSNG.SSC.median.mean.80.90$"90th.percentile" <- comb.0.9$cut.off
# 
# # Get_Sliding_Window_Cutoff_Plots(MSSNG.SSC.median.80.90, "median.80.90")
# Get_Sliding_Window_Cutoff_Plots(MSSNG.SSC.median.mean.80.90, MSSNG.SSC.CH.hits, "median.80.90.mean.spline.probands")


# write.table(sliding.window.df.melt, "./sliding.window.df.melt.tsv", sep="\t", row.names=F, quote=F, col.names=T)
