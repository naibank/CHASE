################################################################################################################################################################################
# 
# FisherTest_CNV_SV_rate_comparison_by_individual_SW.R
# purpose: outputs MSSNG+SSC Fisher's test results using spline of median, 
#           80th, & 90th percentiles for sliding window size bins
# input: MSSNG_SSC_CH_hits (pRec > 0.9 and no pRec filt)
#         /hpf/largeprojects/tcagstor/tcagstor_tmp/shania.wu/CH_count_del_size/data/CH_hits
#        MSSNG.SSC.2.5kb.n7.window.cutoffs.df.tsv (pRec > 0.9 and no pRec filt)
#         /hpf/largeprojects/tcagstor/tcagstor_tmp/shania.wu/CH_count_del_size/data/
# output: e.g. MSSNG.SSC.fisher.spline.allcutoffs.tsv
#        /hpf/largeprojects/tcagstor/tcagstor_tmp/shania.wu/CH_count_del_size/data/
# 
# notes: 
#
...
MSSNG.SSC.2.5kb.n7.window.cutoffs.df <- fread("./data/MSSNG.SSC.2.5kb.n7.window.cutoffs.df.tsv", data.table=F)

MSSNG.SSC.CH.hits.pRec <- fread("./data/CH_hits/MSSNG_SSC_CH_hits_pRec.tsv", data.table=F)
MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df <- fread("./data/MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df", data.table=F)
...
MSSNG.SSC.median.fisher <- Get_Spline_Median_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.window.cutoffs.df, MSSNG.SSC.CH.hits)
MSSNG.SSC.median.fisher.10kb.more <- Get_Spline_Median_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.window.cutoffs.df[which(MSSNG.SSC.2.5kb.n7.window.cutoffs.df$size.bin.start >= 10000),],
  MSSNG.SSC.CH.hits[which(MSSNG.SSC.CH.hits$exSize >= 10000),])

fisher.median <- rbind(MSSNG.SSC.median.fisher, MSSNG.SSC.median.fisher.10kb.more)
...
## pRec > 0.9
MSSNG.SSC.median.fisher.pRec <- Get_Spline_Median_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df, MSSNG.SSC.CH.hits.pRec )
MSSNG.SSC.median.fisher.10kb.more.pRec <- Get_Spline_Median_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df[which(MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df$size.bin.start >= 10000),],
  MSSNG.SSC.CH.hits.pRec[which(MSSNG.SSC.CH.hits.pRec$exSize >= 10000),])

fisher.median.pRec <- rbind(MSSNG.SSC.median.fisher.pRec, MSSNG.SSC.median.fisher.10kb.more.pRec)
new.cols <- data.frame("cut-off" = c("median", "median"),
                       "deletion size range" = c("<30kb", "10-30kb"))
...
MSSNG.SSC.percentile_0.8.fisher <- Get_Spline_0.8_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.window.cutoffs.df, MSSNG.SSC.CH.hits)

MSSNG.SSC.percentile_0.8.fisher.10kb.more <- Get_Spline_0.8_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.window.cutoffs.df[which(MSSNG.SSC.2.5kb.n7.window.cutoffs.df$size.bin.start >= 10000),],
  MSSNG.SSC.CH.hits[which(MSSNG.SSC.CH.hits$exSize >= 10000),])

fisher.percentile_0.8 <- rbind(MSSNG.SSC.percentile_0.8.fisher, MSSNG.SSC.percentile_0.8.fisher.10kb.more)
...
## pRec > 0.9
MSSNG.SSC.percentile_0.8.fisher.pRec <- Get_Spline_0.8_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df, MSSNG.SSC.CH.hits.pRec )
MSSNG.SSC.percentile_0.8.fisher.10kb.more.pRec <- Get_Spline_0.8_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df[which(MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df$size.bin.start >= 10000),],
  MSSNG.SSC.CH.hits.pRec[which(MSSNG.SSC.CH.hits.pRec$exSize >= 10000),])

fisher.percentile_0.8.pRec <- rbind(MSSNG.SSC.percentile_0.8.fisher.pRec, MSSNG.SSC.percentile_0.8.fisher.10kb.more.pRec)
...
MSSNG.SSC.percentile_0.9.fisher <- Get_Spline_0.9_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.window.cutoffs.df, MSSNG.SSC.CH.hits)

MSSNG.SSC.percentile_0.9.fisher.10kb.more <- Get_Spline_0.9_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.window.cutoffs.df[which(MSSNG.SSC.2.5kb.n7.window.cutoffs.df$size.bin.start >= 10000),],
  MSSNG.SSC.CH.hits[which(MSSNG.SSC.CH.hits$exSize >= 10000),])

fisher.percentile_0.9 <- rbind(MSSNG.SSC.percentile_0.9.fisher, MSSNG.SSC.percentile_0.9.fisher.10kb.more)
...
## pRec > 0.9
MSSNG.SSC.percentile_0.9.fisher.pRec <- Get_Spline_0.9_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df, MSSNG.SSC.CH.hits.pRec )
MSSNG.SSC.percentile_0.9.fisher.10kb.more.pRec <- Get_Spline_0.9_Fisher_Test(
  MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df[which(MSSNG.SSC.2.5kb.n7.pRec.window.cutoffs.df$size.bin.start >= 10000),],
  MSSNG.SSC.CH.hits.pRec[which(MSSNG.SSC.CH.hits.pRec$exSize >= 10000),])

fisher.percentile_0.9.pRec <- rbind(MSSNG.SSC.percentile_0.9.fisher.pRec, MSSNG.SSC.percentile_0.9.fisher.10kb.more.pRec)
...
write.table(df.out, "./data/MSSNG.SSC.fisher.spline.allcutoffs.tsv", 
            sep="\t", row.names=F, quote=F, col.names=T)

df.out.pRec <- rbind(fisher.median.pRec, fisher.percentile_0.8.pRec, fisher.percentile_0.9.pRec)
write.table(df.out.pRec, "./data/MSSNG.SSC.fisher.spline.allcutoffs.pRec.tsv", 
            sep="\t", row.names=F, quote=F, col.names=T)
