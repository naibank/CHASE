...
spark1.snv.path <- "/hpf/largeprojects/tcagstor/scratch/kara.han/CHASE/data/SNV/SPARK_WGS_1_all_snv.txt"
spark2.snv.path <- "/hpf/largeprojects/tcagstor/scratch/kara.han/CHASE/data/SNV/SPARK_WGS_2_all_snv.txt"
spark3.snv.path <- "/hpf/largeprojects/tcagstor/scratch/kara.han/CHASE/data/SNV/SPARK_WGS_3_all_snv.txt"
spark4.snv.path <- "/hpf/largeprojects/tcagstor/scratch/kara.han/CHASE/data/SNV/SPARK_WGS_4_all_snv.txt"
...
spark1.cnv_sv.path <- paste(meta.dir, "SPARK_WGS_1.cnvs.svs.10per.cds.mosaic.tagged.tsv", sep = "/")
spark2.cnv_sv.path <- paste(meta.dir, "SPARK_WGS_2.cnvs.svs.10per.cds.mosaic.tagged.tsv", sep = "/")
spark3.cnv_sv.path <- paste(meta.dir, "SPARK_WGS_3.cnvs.svs.10per.cds.mosaic.tagged.tsv", sep = "/")
spark4.cnv_sv.path <- paste(meta.dir, "SPARK_WGS_4.cnvs.svs.10per.cds.mosaic.tagged.tsv", sep = "/")
outfile.path <- "/hpf/largeprojects/tcagstor/scratch/kara.han/CHASE/data/SPARK_CNV_SNV_table.tsv"

spark1.snvs <- fread(spark1.snv.path, data.table = F)
spark2.snvs <- fread(spark2.snv.path, data.table = F)
spark3.snvs <- fread(spark3.snv.path, data.table = F)
spark4.snvs <- fread(spark4.snv.path, data.table = F)
snvs <- rbind(spark1.snvs, spark2.snvs, spark3.snvs, spark4.snvs)
...
all.meta <- rbind(spark1.meta, spark2.meta, spark3.meta, spark4.meta) 
names(all.meta) <- gsub(" ", ".", names(all.meta))
rm(spark1.meta); rm(spark2.meta); rm(spark3.meta); rm(spark4.meta)
nrow(all.meta)
...
spark1.cnv_sv <- fread(spark1.cnv_sv.path, data.table = F)
spark2.cnv_sv <- fread(spark2.cnv_sv.path, data.table = F)
spark3.cnv_sv <- fread(spark3.cnv_sv.path, data.table = F)
spark4.cnv_sv <- fread(spark4.cnv_sv.path, data.table = F)
cnv_sv <- rbind(spark1.cnv_sv, spark2.cnv_sv, spark3.cnv_sv, spark4.cnv_sv)
nrow(cnv_sv)
