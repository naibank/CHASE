################################################################################################################################################################################
#
# unbiased_burden_analysis_MSSNG+SSC_SW.R 
# purpose: outputs burden analysis (BA) results for MSSNG+SSC
# input: sampleIDs to exclude (qcfailed, CRVs, ASD-associated genes), gene exon/set data, 
#         metadata files, files ending with ...SNVsExonicSizes.yaml
# output: one BA results table per data set and comparison, found ats the following diretory:
#         /hpf/largeprojects/tcagstor/tcagstor_tmp/shania.wu/MSSNG+SSC/MSSNG+SSC_BA/data/LogRegResults/
#   ->  used as input in create_DT_result_plots.R
#
# notes: modified from unbiased_burden_analysis_MSSNG+SSC.R (Faraz)
#
##############################################################################################################################################################################
...
setwd("/hpf/largeprojects/tcagstor/tcagstor_tmp/shania.wu/MSSNG+SSC/MSSNG+SSC_BA/data/")
...
Get_Clogit_ANOVA <- function(meta, snvs, ref, mod, type='clogit') {
  ...
  dt.test$totalCHEvents[is.na(dt.test$totalCHEvents)] <- 0 
  dt.test$totalCHEvents_syn[is.na(dt.test$totalCHEvents_syn)] <- 0 
  dt.test$CHrate <- 0
  ...
  if (type == 'clogit') {
    ...
    ref.glm <- glm(paste(gsub("strata\\(Family\\.ID\\)\\ \\+\\ ", "", as.character(ref))[2:3], collapse = "~"), dt.test, family='binomial')
    add.glm <- glm(paste(gsub("strata\\(Family\\.ID\\)\\ \\+\\ ", "", as.character(mod))[2:3], collapse = "~"), dt.test, family='binomial')
  }
  else if (type == 'glm') {
  ...
  # Get odds ratio
  coeff <- add.lm$coefficients["Freq"]
  conf <- confint(add.lm)
  coeff.lower <- conf["Freq", 1]
  coeff.upper <- conf["Freq", 2]
  
  or <- exp(coeff)
  or.lower <- exp(coeff.lower)
  or.upper <- exp(coeff.upper)
  
  # ANOVA
  ano <- anova(ref.lm, add.lm, test = "Chisq")
  
  ano$OR <- or
  ano$OR.lower <- or.lower
  ano$OR.upper <- or.upper
  
  return (list(ref.lm, add.lm, ano, ref.glm, add.glm))
}

Get_Log_Reg <- function(meta, snvs, gene_set_data, exonic_size_data,
                        ref, mod,
                        snv_count_groups = 'parent-child', 
                        type='clogit',
                        apply_SNV_restrictions=T) {
  snvs <- snvs[snvs$X.Sample %in% meta$Sample.ID,]
  snvs <- snvs[!duplicated(snvs[, c("X.Sample", "X.id")]),] ## update from Faraz count
  ...
  if (snv_count_groups == 'proband-unaffected sibling') {
  ...
  all_syn_var <- data.frame(table(snvs$X.Sample[!duplicated(snvs[, c("X.Sample", "X.id")]) &
                                                snvs$effect_priority == "synonymous SNV"]))
  ...
  if (apply_SNV_restrictions) { # Apply to each type alone by separating all variant counts first
    ...
    snvs <- snvs[snvs$gnomAD_pRec >= 0.9 & snvs$gnomAD_oe_lof_upper >= 0.35, ] # pRec > 0.9
  }    
  if (!apply_SNV_restrictions) { 
    SNV_count <- data.frame(Group1 = length(snvs$X.Sample[snvs$X.Sample %in% group1_IDs]),
                            Group2 = length(snvs$X.Sample[snvs$X.Sample %in% group2_IDs]),
                            Group3 = length(snvs$X.Sample[snvs$X.Sample %in% group3_IDs]))
    all_var_count_base <- data.frame(table(snvs$X.Sample[!duplicated(snvs[, c("X.Sample", "X.id")])]))
    ...
    snvs <- snvs[snvs$gnomAD_oe_lof_upper >= 0.35, ] # no pRec cut-off
  }
...
# meta=MSSNG.SSC_meta
# snvs=MSSNG.SSC_parent_proband_proc_SNVs
# gene_set_data=gene_set_data_path
# exonic_size_data=MSSNG.SSC_parent_proband_all_ExonicSizes
# ref=Status ~ strata(Family.ID) + Sex + totalCHEvents
# mod=Status ~ strata(Family.ID) + Sex + totalCHEvents + Freq
# snv_count_groups = 'parent-child'
# type='clogit'
# apply_SNV_restrictions=F
...
MSSNG_parent_proband_SNVsExonicSizes <- yaml::yaml.load_file("MSSNG_parent_proband_SNVsExonicSizes.yaml")
...
MSSNG_parent_proband_SV_SNVsExonicSizes <- yaml::yaml.load_file("MSSNG_parent_proband_SV_SNVsExonicSizes.yaml")
...
SSC_parent_proband_SNVsExonicSizes <- yaml::yaml.load_file("SSC_parent_proband_SNVsExonicSizes.yaml")
...
SSC_parent_proband_SV_SNVsExonicSizes <- yaml::yaml.load_file("SSC_parent_proband_SV_SNVsExonicSizes.yaml")
...
#                                              ref=Status ~ strata(Family.ID) + Sex + totalCHEvents,
#                                              mod=Status ~ strata(Family.ID) + Sex + totalCHEvents + Freq,
#                                              type='clogit') # pRec >= 0.9
# SSC_parent_proband_clogit_nofilt_res <- Get_Log_Reg(SSC_meta,
#                                              SSC_parent_proband_proc_SNVs,
#                                              gene_set_data_path,
#                                              SSC_parent_proband_all_ExonicSizes,
#                                              ref=Status ~ strata(Family.ID) + Sex + exonicSize,
#                                              mod=Status ~ strata(Family.ID) + Sex + exonicSize + Freq,
#                                              type='clogit',
#                                              apply_SNV_restrictions=F) # no pRec filter



### MSSNG+SSC combined parent-proband
meta.cols <- intersect(names(SSC_meta), names(MSSNG_meta))
MSSNG.SSC_meta <- rbind(subset(MSSNG_meta, select = meta.cols), 
                        subset(SSC_meta, select = meta.cols))

MSSNG.SSC_parent_proband_proc_SNVs <- rbind(MSSNG_parent_proband_proc_SNVs,
                                            SSC_parent_proband_proc_SNVs)

MSSNG.SSC_parent_proband_all_ExonicSizes <- rbind(MSSNG_parent_proband_all_ExonicSizes,
                                                 SSC_parent_proband_all_ExonicSizes)

MSSNG.SSC_parent_proband_clogit_res <- Get_Log_Reg(MSSNG.SSC_meta,
                                                   MSSNG.SSC_parent_proband_proc_SNVs,
                                                   gene_set_data_path,
                                                   MSSNG.SSC_parent_proband_all_ExonicSizes,
                                                   ref=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn,
                                                   mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
                                                   type='clogit')

MSSNG.SSC_parent_proband_clogit_nofilt_res <- Get_Log_Reg(MSSNG.SSC_meta,
                                                   MSSNG.SSC_parent_proband_proc_SNVs,
                                                   gene_set_data_path,
                                                   MSSNG.SSC_parent_proband_all_ExonicSizes,
                                                   ref=Status ~ strata(Family.ID) + Sex + exonicSize,
                                                   mod=Status ~ strata(Family.ID) + Sex + exonicSize + Freq,
                                                   type='clogit',
                                                   apply_SNV_restrictions = F) # no pRec cut-off
...
SSC_parent_US_SNVsExonicSizes <- yaml::yaml.load_file("SSC_parent_US_SNVsExonicSizes.yaml")
...
SSC_parent_US_SV_SNVsExonicSizes <- yaml::yaml.load_file("SSC_parent_US_SV_SNVsExonicSizes.yaml")
...
SSC_parent_US_clogit_res <- Get_Log_Reg(SSC_US_meta,
                                        ...
                                        mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
                                        type='clogit') # pRec >= 0.9
SSC_parent_US_clogit_nofilt_res <- Get_Log_Reg(SSC_US_meta,
                                        SSC_parent_US_proc_SNVs,
                                        gene_set_data_path,
                                        SSC_parent_US_all_ExonicSizes,
                                        ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
SSC_proband_US_clogit_res <- Get_Log_Reg(SSC_sibling_meta,
                                         ...
                                         mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
                                         type='clogit') # pRec >= 0.9
SSC_proband_US_clogit_nofilt_res <- Get_Log_Reg(SSC_sibling_meta,
                                         SSC_proband_US_proc_SNVs,
                                         gene_set_data_path,
                                         SSC_proband_US_all_ExonicSizes,
                                         snv_count_groups = 'proband-unaffected sibling',
...
#                                                       mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
#                                                       type='clogit')
# MSSNG_parent_proband_SNV10p_clogit_syn_res <- Get_Log_Reg(MSSNG_meta,
#                                                       MSSNG_parent_proband_proc_SNVs_10p,
#                                                       gene_set_data_path,
#                                                       MSSNG_parent_proband_all_ExonicSizes,
#                                                       ref=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn,
#                                                       mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
#                                                       type='clogit')
# MSSNG_parent_proband_SNV10p_clogit_nofilt_res <- Get_Log_Reg(MSSNG_meta,
#                                                       MSSNG_parent_proband_proc_SNVs_10p,
#                                                       gene_set_data_path,
#                                                       MSSNG_parent_proband_all_ExonicSizes,
#                                                       ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
#                                                      mod=Status ~ strata(Family.ID) + Sex + totalCHEvents + Freq,
#                                                      type='clogit')
# MSSNG_parent_proband_SNV1p_clogit_syn_res <- Get_Log_Reg(MSSNG_meta,
#                                                      MSSNG_parent_proband_proc_SNVs_1p,
#                                                      gene_set_data_path,
#                                                      MSSNG_parent_proband_all_ExonicSizes,
#                                                      ref=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn,
#                                                      mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
#                                                      type='clogit')
# MSSNG_parent_proband_SNV1p_clogit_nofilt_res <- Get_Log_Reg(MSSNG_meta,
#                                                      MSSNG_parent_proband_proc_SNVs_1p,
#                                                      gene_set_data_path,
#                                                      MSSNG_parent_proband_all_ExonicSizes,
#                                                      ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
#                                              mod=Status ~ strata(Family.ID) + Sex + totalCHEvents + Freq,
#                                              type='clogit')
# SSC_parent_proband_SNV10p_clogit_nofilt_res <- Get_Log_Reg(SSC_meta,
#                                                     SSC_parent_proband_proc_SNVs_10p,
#                                                     gene_set_data_path,
#                                                     SSC_parent_proband_all_ExonicSizes,
#                                                     ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
MSSNG.SSC_parent_proband_proc_SNVs_1p <- MSSNG.SSC_parent_proband_proc_SNVs[MSSNG.SSC_parent_proband_proc_SNVs$freq_max <= 0.01,]
MSSNG.SSC_parent_proband_SNV1p_clogit_res <- Get_Log_Reg(MSSNG.SSC_meta,
                                                     MSSNG.SSC_parent_proband_proc_SNVs_1p,
                                                     gene_set_data_path,
                                                     MSSNG.SSC_parent_proband_all_ExonicSizes,
                                                     ref=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn,
                                                     mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
...
MSSNG.SSC_parent_proband_SNV1p_clogit_nofilt_res <- Get_Log_Reg(MSSNG.SSC_meta,
                                                         MSSNG.SSC_parent_proband_proc_SNVs_1p,
                                                         gene_set_data_path,
                                                         MSSNG.SSC_parent_proband_all_ExonicSizes,
                                                         ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
MSSNG.SSC_parent_proband_proc_SNVs_10p <- MSSNG.SSC_parent_proband_proc_SNVs[MSSNG.SSC_parent_proband_proc_SNVs$freq_max <= 0.1,]
MSSNG.SSC_parent_proband_SNV10p_clogit_res <- Get_Log_Reg(MSSNG.SSC_meta,
                                                         MSSNG.SSC_parent_proband_proc_SNVs_10p,
                                                         gene_set_data_path,
                                                         MSSNG.SSC_parent_proband_all_ExonicSizes,
                                                         ref=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn,
                                                         mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
...
MSSNG.SSC_parent_proband_SNV10p_clogit_nofilt_res <- Get_Log_Reg(MSSNG.SSC_meta,
                                                          MSSNG.SSC_parent_proband_proc_SNVs_10p,
                                                          gene_set_data_path,
                                                          MSSNG.SSC_parent_proband_all_ExonicSizes,
                                                          ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
SSC_parent_US_SNV10p_clogit_res <- Get_Log_Reg(SSC_US_meta,
                                        ...
                                        mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
                                        type='clogit')
SSC_parent_US_SNV10p_clogit_nofilt_res <- Get_Log_Reg(SSC_US_meta,
                                               SSC_parent_US_proc_SNVs_10p,
                                               gene_set_data_path,
                                               SSC_parent_US_all_ExonicSizes,
                                               ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
SSC_parent_US_SNV1p_clogit_res <- Get_Log_Reg(SSC_US_meta,
                                               ...
                                               mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
                                               type='clogit')
SSC_parent_US_SNV1p_clogit_nofilt_res <- Get_Log_Reg(SSC_US_meta,
                                              SSC_parent_US_proc_SNVs_1p,
                                              gene_set_data_path,
                                              SSC_parent_US_all_ExonicSizes,
                                              ref=Status ~ strata(Family.ID) + Sex + exonicSize,
...
SSC_proband_US_SNV10p_clogit_res <- Get_Log_Reg(SSC_sibling_meta,
                                         ...
                                         mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
                                         type='clogit')
SSC_proband_US_SNV10p_clogit_nofilt_res <- Get_Log_Reg(SSC_sibling_meta,
                                                SSC_proband_US_proc_SNVs_10p,
                                                gene_set_data_path,
                                                SSC_proband_US_all_ExonicSizes,
                                                snv_count_groups = 'proband-unaffected sibling',
...
SSC_proband_US_SNV1p_clogit_res <- Get_Log_Reg(SSC_sibling_meta,
                                                ...
                                                mod=Status ~ strata(Family.ID) + Sex + totalCHEvents_syn + Freq,
                                                type='clogit')
SSC_proband_US_SNV1p_clogit_nofilt_res <- Get_Log_Reg(SSC_sibling_meta,
                                               SSC_proband_US_proc_SNVs_1p,
                                               gene_set_data_path,
                                               SSC_proband_US_all_ExonicSizes,
                                               snv_count_groups = 'proband-unaffected sibling',
...
###########################################################################################################
#### Data Formatting ######################################################################################
Get_Regression_Slide_Data <- function(regression_results,save_path='./LogRegResults/',
                                      include_base_reg=F) {
  # Saves Faraz LogRes results tables used for Faraz final presentation plots
  res <- list()
...
Get_Regression_Slide_Data_with_OR <- function(regression_results, save_path='./LogRegResultsWithOR/', name) {
  # Saves logreg results with OR and confidence intervals, used for Shania's OR plots
  table.out <- data.frame()
  
  main.res <- regression_results[[1]]
  vartypes <- names(main.res)
  for (i in 1:length(vartypes)) {
    table.row <- data.frame("variant.type" = vartypes[i])
    message(vartypes[i])
    result <- main.res[i]
    if (length(result[[1]]) == 0){
      next
    }
    ano.res <- result[[1]][[3]][2,]
    table.row$P <- ano.res$`P(>|Chi|)`
    table.row$OR <- ano.res$OR
    table.row$OR.upper <- ano.res$OR.upper
    table.row$OR.lower <- ano.res$OR.lower
    
    ref.glm.res <- result[[1]][[4]]
    add.glm.res <- result[[1]][[5]]
    table.row$ref.deviance <- ref.glm.res$deviance
    table.row$ref.null.deviance <- ref.glm.res$null.deviance
    table.row$add.deviance <- add.glm.res$deviance
    table.row$add.null.deviance <- add.glm.res$null.deviance
    
    table.row$G1 <- result[[1]][[6]]
    table.row$G2 <- result[[1]][[7]]
    table.row$G3 <- result[[1]][[8]]
    
    table.out <- rbind(table.out, table.row)
  }
  write.table(table.out, sprintf("./%s/%s_clogit_res.tsv", save_path, name), 
              sep="\t", row.names=F, quote=F, col.names=T)
  return (table.out)
}


## Get OR & confidence interval logres results
MSSNG.SSC.allSNV.slide.data <- Get_Regression_Slide_Data_with_OR(MSSNG.SSC_parent_proband_clogit_res, 
                                                          name = "MSSNG.SSC_parent_proband.allSNV.pRec0.9")
MSSNG.SSC.SNV10p.slide.data <- Get_Regression_Slide_Data_with_OR(MSSNG.SSC_parent_proband_SNV10p_clogit_res, 
                                                                 name = "MSSNG.SSC_parent_proband.SNV10p.pRec0.9")
MSSNG.SSC.SNV1p.slide.data <- Get_Regression_Slide_Data_with_OR(MSSNG.SSC_parent_proband_SNV1p_clogit_res, 
                                                                name = "MSSNG.SSC_parent_proband.SNV1p.pRec0.9")

MSSNG.SSC.allSNV.nofilt.slide.data <- Get_Regression_Slide_Data_with_OR(MSSNG.SSC_parent_proband_clogit_nofilt_res, 
                                                                 name = "MSSNG.SSC_parent_proband.allSNV.nofilt")
MSSNG.SSC.SNV10p.nofilt.slide.data <- Get_Regression_Slide_Data_with_OR(MSSNG.SSC_parent_proband_SNV10p_clogit_nofilt_res, 
                                                                 name = "MSSNG.SSC_parent_proband.SNV10p.nofilt")
MSSNG.SSC.SNV1p.nofilt.slide.data <- Get_Regression_Slide_Data_with_OR(MSSNG.SSC_parent_proband_SNV1p_clogit_nofilt_res, 
                                                                name = "MSSNG.SSC_parent_proband.SNV1p.nofilt")

SSC_parent_US.allSNV.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_parent_US_clogit_res, 
                                                                      name = "SSC_parent_US.allSNV.pRec0.9")
SSC_parent_US.SNV10p.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_parent_US_SNV10p_clogit_res, 
                                                                      name = "SSC_parent_US.SNV10p.pRec0.9")
SSC_parent_US.SNV1p.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_parent_US_SNV1p_clogit_res, 
                                                                      name = "SSC_parent_US.SNV1p.pRec0.9")

SSC_parent_US.allSNV.nofilt.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_parent_US_clogit_nofilt_res, 
                                                                      name = "SSC_parent_US.allSNV.nofilt")
SSC_parent_US.SNV10p.nofilt.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_parent_US_SNV10p_clogit_nofilt_res, 
                                                                      name = "SSC_parent_US.SNV10p.nofilt")
SSC_parent_US.SNV1p.nofilt.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_parent_US_SNV1p_clogit_nofilt_res, 
                                                                     name = "SSC_parent_US.SNV1p.nofilt")

SSC_proband_US.allSNV.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_proband_US_clogit_res, 
                                                                      name = "SSC_proband_US.allSNV.pRec0.9")
SSC_proband_US.SNV10p.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_proband_US_SNV10p_clogit_res, 
                                                                      name = "SSC_proband_US.SNV10p.pRec0.9")
SSC_proband_US.SNV1p.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_proband_US_SNV1p_clogit_res, 
                                                                     name = "SSC_proband_US.SNV1p.pRec0.9")

SSC_proband_US.allSNV.nofilt.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_proband_US_clogit_nofilt_res, 
                                                                       name = "SSC_proband_US.allSNV.nofilt")
SSC_proband_US.SNV10p.nofilt.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_proband_US_SNV10p_clogit_nofilt_res, 
                                                                       name = "SSC_proband_US.SNV10p.nofilt")
SSC_proband_US.SNV1p.nofilt.slide.data <-  Get_Regression_Slide_Data_with_OR(SSC_proband_US_SNV1p_clogit_nofilt_res, 
                                                                      name = "SSC_proband_US.SNV1p.nofilt")
...
#                    MSSNG.SSC_parent_proband_clogit_res=MSSNG.SSC_parent_proband_clogit_res,
...
#                                MSSNG.SSC_parent_proband_SNV10p_clogit_res = MSSNG.SSC_parent_proband_SNV10p_clogit_res,
#                                MSSNG.SSC_parent_proband_SNV1p_clogit_res = MSSNG.SSC_parent_proband_SNV1p_clogit_res,
