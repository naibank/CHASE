...
setwd("/Users/shaniawu/Desktop/CHASE/SPARK_BA")
...
gene_exon_data <- data.table::fread("../gene_data/hg38_refGene_20200708.exon.txt", data.table = F)
gene_set_data_path <- "../gene_data/gsMain_PGC_2021.RData"
...
for (i in 1:3) {
  ...
  SPARK_parent_proband_SNVsExonicSizes_curr <- yaml::yaml.load_file(SPARK_SNVsExonicSizes_path)
    # SNVs - SNVsExonicSizes_curr[[1]]
    # ExonicSizes - SNVsExonicSizes_curr[[2]]
    # CNVs - SNVsExonicSizes_curr[[3]]
...
## Filter out potential false positive SNV calls (<50bp from another SNV)
...
for (i in 1:3) {
  SPARK_sibling_metadata_path <- paste("./Data/SPARK_WGS_",i,"_metadata_relfixed.tsv",sep='')
...
Generate_Indiv_Count_Bar_Plots <- function(data, name, save_path='./IndivCountBarPlots/',
...
write.csv(SPARK_gene_summary, './SPARK_proband_genes_missense_summary.csv')
