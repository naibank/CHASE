...
# Fix metadata relation for SPARK WGS 1
Fix_SPARK_Metadata_Relation_WGS1 <- function(metadata.path, newname) {
  meta <- read.delim(metadata.path, stringsAsFactors = F)
  meta$Relation[meta$Affection == 2] <- 'proband'
  meta$Relation[meta$Affection == 1 & meta$Relation=='sibling'] <- 'unaffected sibling'
  
  write.table(meta, newname, sep="\t", row.names=F, quote=F, col.names=T)
}

Fix_SPARK_Metadata_Relation_WGS1('/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/SPARK_WGS_1_metadata.tsv',
                                 '/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/SPARK_WGS_1_metadata_relfixed.tsv')
...
CNVfilter.procedure <- function(CNVfile.path){
  ...
  colnames(CNVs)[126] <- 'Family.ID'
  colnames(CNVs)[127] <- 'Sample.ID'
  colnames(CNVs)[136] <- 'Mother.ID'
  colnames(CNVs)[137] <- 'Father.ID'  
  # Selecting only CNV deletions
  CNVs <- CNVs %>% dplyr::filter(variantTypeAnn == "del")
  ...
  CNVs <- CNVs %>% dplyr::filter(Sample_QC == "pass")
...
CNVs_10Percent <- CNVfilter.procedure(CNVfile.path = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/CNVs/CNVs.SPARK_WGS_1.freq_1percent.HQR.tsv")

## STEP 2: proband filtering 
# Exclude families when proband also has an ASD affected parent
Family_to_exclude <- as_tibble(read.delim("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/SPARK_WGS_1_metadata_relfixed.tsv", stringsAsFactors = F)) %>%
...
## Get probands using 10 Percent CNV database
probands_10P <- LocateProbands(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/SPARK_WGS_1_metadata_relfixed.tsv",
...
Unaffected_siblings10P <- Locate_Usiblings(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/SPARK_WGS_1_metadata_relfixed.tsv",
                                           CNV.database = CNVs_10Percent,
                                           Family_to_exclude = Family_to_exclude,
                                           Subjects_to_exclude = Subjects_to_exclude)
...
# Get samples of probands' parents
parents_10P <- LocateParents(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/SPARK_WGS_1_metadata_relfixed.tsv",
...
Get_CH_Events <- function(probands, CNVdf, SNVfolder){
  ...
  for(i in 1:nrow(probands)){
    ...
    probandFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/", SNVfolder, "/", 
    ...
    motherFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/", SNVfolder, "/",
    ...
    fatherFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_1/", SNVfolder, "/",
        ...
        expr = {
          ...
          if(nrow(snvs) > 0 & nrow(CH_Data[[proband]][[cnv]]) > 0){
            ...
            snvs <- as_tibble(snvs) %>% mutate(LoF = ifelse(str_detect(effect_impact_str, "Stop_gain-High") == T |
                                                              ...
                                                              str_detect(effect_impact_str, "Splice_site-High") == T |
...
print("getting SPARKWGS1 probands data")
Get_CH_Events(probands = probands_10P,
              CNVdf = CNVs_10Percent,
              SNVfolder = "exonic+splicing") %>%
  write_yaml(.,"SPARKWGS1_CH_Data_CNV1P_SNV.yaml")
...
Get_CH_Events(probands = Unaffected_siblings10P,
              CNVdf = CNVs_10Percent,
              SNVfolder = "exonic+splicing") %>%
  write_yaml(.,"SPARKWGS1_US_CH_Data_CNV1P_SNV.yaml")
