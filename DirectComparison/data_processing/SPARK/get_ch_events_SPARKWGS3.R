...
# Fix metadata relation for SPARK WGS 2 and 3
Fix_SPARK_Metadata_Relation <- function(metadata.path, newname) {
  meta <- read.delim(metadata.path, stringsAsFactors=F)
  meta$Relation[meta$Affection == 2] <- 'proband'
  meta$Relation[meta$Affection == 1 & meta$Relation=='child'] <- 'unaffected sibling'
  
  write.table(meta, newname, sep="\t", row.names=F, quote=F, col.names=T)
}

Fix_SPARK_Metadata_Relation('/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/SPARK_WGS_3_metadata.tsv',
                            '/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/SPARK_WGS_3_metadata_relfixed.tsv')
...
CNVs_10Percent <- CNVfilter.procedure(CNVfile.path = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/CNVs/CNVs.SPARK_WGS_3.freq_1percent.HQR.tsv")

## STEP 2: proband filtering 
# Exclude families when proband also has an ASD affected parent
Family_to_exclude <- as_tibble(read.delim("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/SPARK_WGS_3_metadata_relfixed.tsv", stringsAsFactors = F)) %>%
...
probands_10P <- LocateProbands(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/SPARK_WGS_3_metadata_relfixed.tsv",
...
Unaffected_siblings10P <- Locate_Usiblings(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/SPARK_WGS_3_metadata_relfixed.tsv",
                                           CNV.database = CNVs_10Percent,
                                           Family_to_exclude = Family_to_exclude,
                                           Subjects_to_exclude = Subjects_to_exclude)
...
parents_10P <- LocateParents(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/SPARK_WGS_3_metadata_relfixed.tsv",
...
Get_CH_Events <- function(probands, CNVdf, SNVfolder){
  ...
  for(i in 1:nrow(probands)){
    ...
    probandFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/", SNVfolder, "/", 
    ...
    motherFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/", SNVfolder, "/",
    ...
    fatherFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/SPARK_WGS_3/", SNVfolder, "/",
...
print("getting SPARKWGS3 probands data")
Get_CH_Events(probands = probands_10P,
              CNVdf = CNVs_10Percent,
              SNVfolder = "exonic+splicing") %>%
  write_yaml(.,"SPARKWGS3_CH_Data_CNV1P_SNV.yaml")
...
Get_CH_Events(probands = Unaffected_siblings10P,
              CNVdf = CNVs_10Percent,
              SNVfolder = "exonic+splicing") %>%
  write_yaml(.,"SPARKWGS3_US_CH_Data_CNV1P_SNV.yaml")
