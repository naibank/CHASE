...
CNVfilter.procedure.CG <- function(CNVfile.path){
  CNVs <- fread(CNVfile.path, stringsAsFactors = FALSE) 
  colnames(CNVs)[2] <- 'CHROM'
  colnames(CNVs)[3] <- 'START'
  colnames(CNVs)[4] <- 'END'
  colnames(CNVs)[5] <- 'sv_type'
  colnames(CNVs)[121] <- 'Family.ID'
  colnames(CNVs)[122] <- 'Sample.ID'
  colnames(CNVs)[131] <- 'Mother.ID'
  colnames(CNVs)[132] <- 'Father.ID'
  # Selecting only CNV deletions
  CNVs <- CNVs[CNVs$sv_type == 'deletion', ]
  # Filtering out CNVs with "NA", ambiguous and one parent sequenced inheritance
  # CNVs <- CNVs  %>% dplyr::filter(!is.na(Inheritance)) 
  # CNVs <- CNVs  %>% dplyr::filter(Inheritance != "Ambiguous")
  # CNVs <- CNVs  %>% dplyr::filter(Inheritance != "One_parent_sequenced") 
...
CNVs_10Percent <- CNVfilter.procedure(CNVfile.path = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/MSSNG/SVs.ILMN/SVs.MSSNG_ILMN.freq_1percent.HQR.IntFreq.tsv")

# Getting CNVs (with different MAF limits) for samples sequenced by CG platform
CNVs_10Percent.CG <- CNVfilter.procedure.CG(CNVfile.path = "/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/MSSNG/SVs.CG/SVs.MSSNG_CG.freq_1percent.HQR.IntFreq.tsv")
...
Get_CH_Events <- function(probands, CNVdf, SNVfolder){
  ...
  for(i in 1:nrow(probands)){
    ...
    # message(paste('proband ID:',proband))
    # message(head(probands))
    # write.csv(probands, 'testprob.csv')
    # Skip probands with no parent ID information
    if (proband == '-' | mother[1] == '-' | father[1] == '-') {
      next
    }
...
Get_CH_Events(probands = probands_10P,
  ...
  write_yaml(.,"MSSNG_ILMN_CH_Data_SV1P_SNV.yaml")
...
Get_CH_Events(probands = probands_10P.CG,
  ...
  write_yaml(.,"MSSNG_CG_CH_Data_SV1P_SNV.yaml")
