...
setwd("/hpf/largeprojects/tcagstor/tcagstor_tmp/farazali/DirectComparison")
...
Get_SNVs_With_Exonic_Size <- function(CNVs, SNVs, filtered_meta, genes_data, 
                                      with_CNV = T, is_CG=F, ds='MSSNG', CNV_freq_filter=1, 
  ...
  if (length(CNVs) > 0) {
    # CNVs$cnvFreq <- ifelse(with_CNV, Determine_CNV_MAF(CNVs), Determine_SV_MAF(CNVs, is_CG))
      ...
      if (CNVs$Sample.ID[1] %in% filtered_meta$Sample.ID) { # Implicitly apply all filtering
        ...
        if (nrow(genes) > 0) {
          ...
          if (nrow(SNVs) > 0) {
            ...
            SNVs$CHfreq <- (SNVs$cnvFreq/100) * SNVs$freq_max
...
Process_Parent_Child_CNV_SNV_Data <- function(data_sets, meta_data, genes_data,
  ...
  for (j in 1:length(data_sets)) {
    ...
    for(i in 1:length(data)){
      ...
      if (nrow(childCNVs) > 0) {
        childData <- Get_SNVs_With_Exonic_Size(childCNVs, childSNVs, meta_data, genes_data, with_CNV,
                                               is_CG=is_CG, ds, CNV_freq_filter)
        ...
        if (!is_CG) {
          ...
          all_CNVs_CG <- dplyr::bind_rows(all_CNVs_CG, childCNVs)
        }
      }
      if (nrow(motherCNVs) > 0) {
        motherData <- Get_SNVs_With_Exonic_Size(motherCNVs, motherSNVs, meta_data, genes_data, with_CNV, 
                                                is_CG=is_CG, ds, CNV_freq_filter)
        ...
        if (!is_CG) {
          all_CNVs <- dplyr::bind_rows(all_CNVs, motherCNVs)
        }
        else {
          all_CNVs_CG <- dplyr::bind_rows(all_CNVs_CG, motherCNVs)
        }
      }
      if (nrow(fatherCNVs) > 0) {
        fatherData <- Get_SNVs_With_Exonic_Size(fatherCNVs, fatherSNVs, meta_data, genes_data, with_CNV, 
                                                is_CG=is_CG, ds, CNV_freq_filter)
        ...
        if (!is_CG) {
          all_CNVs <- dplyr::bind_rows(all_CNVs, fatherCNVs)
        }
        else {
          all_CNVs_CG <- dplyr::bind_rows(all_CNVs_CG, fatherCNVs)
        }
      }
  ...
  return (list(all_SNVs, all_exonic_sizes, all_CNVs, all_CNVs_CG))
}


gene_exon_data <- data.table::fread("./Data/hg38_refGene_20200708.exon.txt", data.table = F)
gene_set_data_path <- "./Data/gsMain_PGC_2021.RData"
### MSSNG CNV Parent-Proband
MSSNG_metadata_path <- "./Data/MSSNG_metadata.tsv"
MSSNG_meta <- Get_Filtered_Metadata(MSSNG_metadata_path, child_relation = 'proband')

MSSNG_ILMN_data <- yaml::yaml.load_file("./Data/MSSNG_ILMN_CH_Data_CNV10P_SNV.yaml")
MSSNG_CG_data <- yaml::yaml.load_file("./Data/MSSNG_CG_CH_Data_CNV10P_SNV.yaml")
MSSNG_all_data <- list(MSSNG_ILMN_data, MSSNG_CG_data)
MSSNG_parent_proband_SNVsExonicSizes <- Process_Parent_Child_CNV_SNV_Data(MSSNG_all_data,
                                                                          MSSNG_meta,
                                                                          gene_exon_data,
                                                                          CNV_freq_filter=0.01,
                                                                          ds='MSSNG')
write_yaml(MSSNG_parent_proband_SNVsExonicSizes, "MSSNG_parent_proband_SNVsExonicSizes.yaml")

### MSSNG SV Parent-Proband (Make sure STARTAnn and ENDAnn cols are renamed)
MSSNG_SV_ILMN_data <- yaml::yaml.load_file("./Data/MSSNG_ILMN_CH_Data_SV1P_SNV.yaml")
MSSNG_SV_CG_data <- yaml::yaml.load_file("./Data/MSSNG_CG_CH_Data_SV1P_SNV.yaml")
MSSNG_SV_all_data <- list(MSSNG_SV_ILMN_data, MSSNG_SV_CG_data)
MSSNG_SV_parent_proband_SNVsExonicSizes <- Process_Parent_Child_CNV_SNV_Data(MSSNG_SV_all_data,
                                                                             MSSNG_meta,
                                                                             gene_exon_data,
                                                                             with_CNV = F,
                                                                             CNV_freq_filter=0.01,
                                                                             ds='MSSNG')
write_yaml(MSSNG_SV_parent_proband_SNVsExonicSizes, "MSSNG_parent_proband_SV_SNVsExonicSizes.yaml")
message("done")
