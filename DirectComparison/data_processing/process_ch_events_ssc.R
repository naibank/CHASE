...
### SSC CNV Parent-Proband
SSC_metadata_path <- "./Data/SSC_metadata.tsv"
SSC_meta <- Get_Filtered_Metadata(SSC_metadata_path, child_relation = 'proband')

SSC_SV_data <- yaml::yaml.load_file("./Data/SSC_CH_Data_CNV10P_SNV.yaml")
SSC_all_data <- list(SSC_SV_data)
SSC_parent_proband_SNVsExonicSizes <- Process_Parent_Child_CNV_SNV_Data(SSC_all_data,
                                                                          SSC_meta,
                                                                          gene_exon_data,
                                                                          CNV_freq_filter=0.01,
                                                                          ds='SSC')
write_yaml(SSC_parent_proband_SNVsExonicSizes, "SSC_parent_proband_SNVsExonicSizes.yaml")

### SSC SV Parent-Proband (Make sure STARTAnn and ENDAnn cols are renamed)
SSC_SV_data <- yaml::yaml.load_file("./Data/SSC_CH_Data_SV1P_SNV.yaml")
SSC_SV_all_data <- list(SSC_SV_data)
SSC_SV_parent_proband_SNVsExonicSizes <- Process_Parent_Child_CNV_SNV_Data(SSC_SV_all_data,
                                                                             SSC_meta,
                                                                             gene_exon_data,
                                                                             with_CNV = F,
                                                                             CNV_freq_filter=0.01,
                                                                             ds='SSC')
write_yaml(SSC_SV_parent_proband_SNVsExonicSizes, "SSC_parent_proband_SV_SNVsExonicSizes.yaml")
message("done")
