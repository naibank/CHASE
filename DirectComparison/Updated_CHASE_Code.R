...
CNVs_10Percent <- CNVfilter.procedure(CNVfile.path = "/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/CNVs.ILMN/CNVs.MSSNG_ILMN.freq_10percent.HQR.tsv")

## Getting CNVs (with different MAF limits) for samples sequenced by CG platform
CNVs_10Percent.CG <- CNVfilter.procedure(CNVfile.path = "/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/CNVs.CG/CNVs.MSSNG_CG.freq_10percent.HQR.tsv")

###STEP 2: proband filtering 
##Exclude families when proband also has an ASD affected parent
Family_to_exclude <- as_tibble(read.delim("/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/MSSNG_metadata.tsv", stringsAsFactors = F)) %>%
...
probands_10P <- LocateProbands(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/MSSNG_metadata.tsv",
...
probands_10P.CG <- LocateProbands(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/MSSNG_metadata.tsv",
...
parents_10P <- LocateParents(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/MSSNG_metadata.tsv",
...
parents_10P.CG <- LocateParents(metadata.filepath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/MSSNG_metadata.tsv",
...
Get_CH_Events <- function(probands, CNVdf, CNVpath, SNVfolder){
  allCNVs <- as_tibble(fread(CNVpath, fill = T)) %>% dplyr::filter(variantTypeAnn == "deletion" & 
                                                                     (cds_symbol != "" & !is.na(cds_symbol)) &
                                                                     Sample_QC == "ok" & !CHROM %in% c("chrX", "chrY"))
  
  colnames(allCNVs)[1] <- "Sample.ID"
  ...
  for(i in 1:nrow(probands)){
    ...
    probandFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/", SNVfolder, "/", 
    ...
    motherFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/", SNVfolder, "/",
    ...
    fatherFilepath <- str_c(c("/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/", SNVfolder, "/",
...
Get_CH_Events(probands = probands_10P.CG,
              ...
              CNVpath = "/hpf/largeprojects/tcagstor/tcagstor_tmp/alex.chan/MSSNG/CNVs.CG/CNVs.MSSNG_CG.freq_10percent.HQR.tsv",
